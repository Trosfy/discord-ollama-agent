...
def split_message(
    ...
    Adds part indicators if configured.
    ...
    # Reserve space for part indicators if enabled
    # "[Part 999/999]\n" = 17 chars worst case for up to 999 parts
    indicator_overhead = 20 if settings.DISCORD_SPLIT_MESSAGE_INDICATOR else 0
    effective_max_length = max_length - indicator_overhead
    ...
    for line in lines:
        ...
        if line_length > effective_max_length:
            ...
            chunks.extend(_split_long_line(line, effective_max_length))
            continue

        # Check if adding this line exceeds limit
        if current_length + line_length > effective_max_length:
            # Save current chunk
            if current_chunk:
                chunks.append('\n'.join(current_chunk))
    ...
    if settings.DISCORD_SPLIT_MESSAGE_INDICATOR and len(chunks) > 1:
