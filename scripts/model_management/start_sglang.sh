...
cd "$PROJECT_ROOT"
docker compose -f docker-compose.infra.yml -f docker-compose.app.yml --profile performance up -d sglang-server
...
while true; do
    ...
    if [ $ELAPSED -ge $TIMEOUT ]; then
        echo "⚠️  Startup timeout after 10 minutes - may still be initializing"
        break
    fi
